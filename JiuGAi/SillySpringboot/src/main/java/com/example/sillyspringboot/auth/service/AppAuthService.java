...
public class AppAuthService {
    ...
    public AppAuthSessionResponse loginWithH5Account(H5AccountLoginRequest request) {
        return loginWithH5Account(request, null);
    }

    @Transactional
    public AppAuthSessionResponse loginWithH5Account(H5AccountLoginRequest request, String clientUid) {
    ...
    private void bindH5ClientUidToUser(String clientUid, long userId) {
        String normalized = clientUid == null ? "" : clientUid.trim();
        if (normalized.isBlank() || normalized.startsWith("h5u_")) {
            return;
        }
        AppH5ClientUid existing = h5ClientUidMapper.findByClientUid(normalized);
        if (existing == null) {
            AppH5ClientUid row = new AppH5ClientUid();
            row.setClientUid(normalized);
            row.setUserId(userId);
            try {
                h5ClientUidMapper.insert(row);
            } catch (DuplicateKeyException ex) {
                h5ClientUidMapper.updateUserIdByClientUid(normalized, userId);
            }
            return;
        }
