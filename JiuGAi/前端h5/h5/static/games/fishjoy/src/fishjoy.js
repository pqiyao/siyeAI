...
{
	if(typeof window.FishjoyFitLayout == "function")
	{
		return window.FishjoyFitLayout();
	}
	var viewportWidth = window.innerWidth || 980;
	var viewportHeight = window.innerHeight || 545;
	var rotated = viewportHeight > viewportWidth;
	return {
...
{	
	...
	this.view = getGameView();
	this.width = this.view.width || window.innerWidth || 980;
	this.height = this.view.height || window.innerHeight || 545;
	Q.addMeta({name:"viewport", content:"width=device-width, initial-scale=1.0, minimum-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover"});
...
game.startup = function()
{
	...
	this.patchStageEvent(this.stage);
...
game.getEventPoint = function(e)
{
	var view = window.FISHJOY_VIEW || this.view || getGameView();
	var pageX = e.pageX != null ? e.pageX : e.clientX + window.pageXOffset;
	var pageY = e.pageY != null ? e.pageY : e.clientY + window.pageYOffset;
	var centerX = (view.viewportWidth || window.innerWidth) / 2;
	var centerY = (view.viewportHeight || window.innerHeight) / 2;
	var dx = pageX - centerX;
	var dy = pageY - centerY;
	var scaleX = view.scaleX || 1;
	var scaleY = view.scaleY || 1;
	if(view && view.rotated)
	{
		return {
			x: this.width / 2 + dy / scaleX,
			y: this.height / 2 - dx / scaleY
		};
	}
	return {
		x: this.width / 2 + dx / scaleX,
		y: this.height / 2 + dy / scaleY
	};
};

game.patchStageEvent = function(stage)
{
	var original = stage._onEvent;
	var me = this;
	stage._onEvent = function(e)
	{
		var view = window.FISHJOY_VIEW || me.view;
		if(!view)
		{
			original.call(stage, e);
			return;
		}
		var point = me.getEventPoint(e);
		var target = stage.getObjectUnderPoint(point.x, point.y, true);
		var lastTarget = stage._eventTarget;
		e.eventX = point.x;
		e.eventY = point.y;
		if(lastTarget != null && lastTarget != target)
		{
			e.lastEventTarget = lastTarget;
			var outType = e.type == "mousemove" ? "mouseout" : e.type == "touchmove" ? "touchout" : null;
			if(outType) lastTarget._onEvent({type:outType});
			stage._eventTarget = null;
		}
		if(target != null && target.eventEnabled)
		{
			e.eventTarget = stage._eventTarget = target;
			target._onEvent(e);
		}
		if(stage.onEvent != null) stage.onEvent(e);
	};
};
...
game.initUI = function()
{
	...
	this.uiScale = Math.min(this.width / this.bottom.width, this.height / 320, 1);
	this.bottom.scaleX = this.bottom.scaleY = this.uiScale;
	this.bottom.x = (this.width - this.bottom.width * this.uiScale) >> 1;
	this.bottom.y = this.height - this.bottom.height * this.uiScale + 2;
