...
const CHAT_SOURCE_JSONL = Symbol('chatSourceJsonl');
...
export async function trySaveChat(chatData, filePath, skipIntegrityCheck = false, handle, cardName, backupDirectory) {
    ...
    if (chatData?.[CHAT_SOURCE_JSONL] === jsonlData) {
        return false;
    }
    tryWriteFileSync(filePath, jsonlData);
    rememberChatSource(chatData, jsonlData);
    getBackupFunction(handle)(backupDirectory, cardName, jsonlData);
    return true;
}

function rememberChatSource(chatData, jsonlData) {
    if (!Array.isArray(chatData) || !Object.isExtensible(chatData)) {
        return;
    }
    Object.defineProperty(chatData, CHAT_SOURCE_JSONL, {
        value: jsonlData,
        configurable: true,
        writable: true,
    });
...
function resolveChatLocation(directories, avatarUrl, fileName = '') {
    const safeChatFileName = fileName ? sanitize(`${String(fileName)}.jsonl`) : '';

    if (safeChatFileName && fs.existsSync(directories.chats)) {
        ...
        if (existingDir) {
            return {
                cardName: existingDir,
                directoryPath: path.join(directories.chats, existingDir),
    ...
    const rawCardName = canonicalAvatarUrl.replace(/\.png$/i, '');
    const candidates = buildNameCandidates(rawCardName);
    const chatDirs = fs.existsSync(directories.chats)
        ? fs.readdirSync(directories.chats, { withFileTypes: true })
            .filter(entry => entry.isDirectory())
            .map(entry => entry.name)
        : [];
    const matchedCardName = findMatchingEntry(candidates, chatDirs, { caseInsensitive: false }) || rawCardName;
    return {
        cardName: matchedCardName,
        directoryPath: path.join(directories.chats, matchedCardName),
        chatFilePath: safeChatFileName ? path.join(directories.chats, matchedCardName, safeChatFileName) : '',
    };
}
...
router.post('/save', validateAvatarUrlMiddleware, async function (request, response) {
    try {
        ...
        const cardName = location.cardName;
        const chatFilePath = location.chatFilePath;
...
export function getChatData(chatFilePath) {
    ...
    if (chatJSON.length > 0) {
        ...
        rememberChatSource(chatData, chatJSON);
    } else {
...
router.post('/get', validateAvatarUrlMiddleware, function (request, response) {
    try {
        ...
        return response.send(getChatData(location.chatFilePath));
    } catch (error) {
        console.error(error);
        return response.send({});
    }
