...
> [返回项目首页](../README.md) · [部署说明](DEPLOYMENT.md)
...
`backend/` 是业务边界和统一 API 入口。前端不直接持有模型、支付或数据库密钥；敏感配置由后端通过环境变量读取。
...
模块内的构建产物、依赖目录、运行数据和本地密钥不应提交到仓库。生产部署要求见 [部署说明](DEPLOYMENT.md)。
