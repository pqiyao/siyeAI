		...
		<template v-else-if="char">
			<view class="hero-stage">
				...
				<view class="hero-identity">
					<text class="hero-eyebrow">{{ primaryGameplay }}</text>
					<text class="hero-name">{{ char.nickname }}</text>
					<text class="hero-one-line">{{ detailOneLiner }}</text>
					<view v-if="heroTags.length" class="hero-chip-row">
						<text v-for="(tag, idx) in heroTags" :key="'hero_tag_' + idx" class="hero-chip">{{ tag }}</text>
					</view>
				</view>
			...
			<scroll-view scroll-y class="page-scroll" :show-scrollbar="false" enable-back-to-top>
				...
				<view class="sheet-shell">
					<view class="sheet-card">
						...
						<view class="panel-glass lead-panel">
							<view class="lead-copy">
								<text class="lead-label">{{ detailLocalText.leadLabel }}</text>
								<text class="lead-title">{{ detailOneLiner }}</text>
								<text class="lead-desc">{{ publicSummary }}</text>
							</view>
							<view v-if="quickMetaItems.length" class="lead-meta-grid">
								<view v-for="item in quickMetaItems" :key="item.label" class="lead-meta-item">
									<text class="lead-meta-label">{{ item.label }}</text>
									<text class="lead-meta-value">{{ item.value }}</text>
								</view>
							</view>
							<view v-if="openingPreview" class="opening-preview">
								<view class="opening-preview-head">
									<text class="opening-preview-title">{{ detailLocalText.openingTitle }}</text>
									<text class="opening-preview-badge">{{ detailLocalText.openingBadge }}</text>
								</view>
								<text class="opening-preview-text">{{ openingPreview }}</text>
							</view>
						</view>
...
export default {
	...
	computed: {
		...
		detailLocalText() {
			...
			const map = {
				'zh-cn': {
					...
					tagsLabel: '标签',
					anonymous: '匿名',
					openingFallback: '进入私聊后会展示角色开场，并逐步展开完整设定。'
				},
				'zh-hk': {
					leadLabel: '開聊前看這個',
					openingTitle: '開場預覽',
					...
					tagsLabel: '標籤',
					anonymous: '匿名',
					openingFallback: '進入私聊後會展示角色開場，並逐步展開完整設定。'
				},
				en: {
					leadLabel: 'Before you start',
					openingTitle: 'Opening Preview',
					openingBadge: 'First line',
					gameplayLabel: 'Gameplay',
					modesLabel: 'Modes',
					tagsLabel: 'Tags',
					anonymous: 'Anonymous',
					openingFallback: 'The private chat reveals the opening line and the full setup gradually.'
				}
			};
			return map[code] || map['zh-cn'];
		},
		...
		primaryGameplay() {
			return (this.char && this.char.gameplay_type) || this.uiText.gameplayFallback || this.detailPatchText.modeRoleplay;
		},
		heroTags() {
		...
		quickMetaItems() {
			if (!this.char) return [];
			const list = [];
			if (this.primaryGameplay) {
				list.push({
					label: this.detailLocalText.gameplayLabel,
					value: this.truncateText(this.primaryGameplay, 18)
				});
			}
			if (this.modeSummary) {
				list.push({
					label: this.detailLocalText.modesLabel,
					value: this.truncateText(this.modeSummary, 20)
				});
			}
			const tags = this.heroTags.join(this.detailPatchText.tagJoiner);
			if (tags) {
				list.push({
					label: this.detailLocalText.tagsLabel,
					value: this.truncateText(tags, 22)
				});
			}
...
.hero-name {
	...
	font-weight: 900;
	color: #ffffff;
	text-shadow: 0 5rpx 18rpx rgba(6, 14, 26, 0.36);
	overflow: hidden;
	text-overflow: ellipsis;
	white-space: nowrap;
}
...
.hero-one-line {
	...
	text-shadow: 0 3rpx 12rpx rgba(6, 14, 26, 0.32);
	display: -webkit-box;
	overflow: hidden;
	-webkit-line-clamp: 2;
...
.opening-preview {
	...
	border-radius: 24rpx;
	background:
		linear-gradient(135deg, rgba(52, 143, 184, 0.16) 0%, rgba(236, 72, 153, 0.12) 100%),
		rgba(255, 255, 255, 0.04);
	border: 1rpx solid rgba(255, 255, 255, 0.1);
}

.opening-preview-head {
	display: flex;
	align-items: center;
	justify-content: space-between;
	gap: 14rpx;
...
/* Light clover tavern detail refresh. */
.page,
.loading-wrap,
.empty-wrap {
	background-image: url('/static/login.png');
...
.hero-mask {
	background: linear-gradient(180deg, rgba(255, 255, 255, 0.02) 0%, rgba(255, 255, 255, 0.08) 48%, rgba(255, 255, 255, 0.18) 100%);
}

.hero-fade {
	background: linear-gradient(180deg, rgba(255, 255, 255, 0) 0%, rgba(236, 249, 254, 0.32) 100%);
}

.chrome {
	background: linear-gradient(180deg, rgba(255, 255, 255, 0.56) 0%, rgba(255, 255, 255, 0.14) 72%, rgba(255, 255, 255, 0) 100%);
}
...
.chrome-btn {
	...
	box-shadow: 0 10rpx 22rpx rgba(67, 112, 142, 0.1);
}
...
.sheet-handle {
	background: rgba(52, 143, 184, 0.24);
}
...
.tag-chip,
.mode-icon {
	background: rgba(220, 247, 251, 0.74);
	border-color: rgba(88, 189, 210, 0.28);
}
...
.mode-rec {
	...
	border-color: rgba(245, 190, 86, 0.24);
}

.cta-shell {
	background: linear-gradient(180deg, rgba(255, 244, 248, 0) 0%, rgba(255, 244, 248, 0.08) 34%, rgba(255, 244, 248, 0.16) 100%);
}
...
.cta-mini {
	...
	box-shadow: 0 16rpx 32rpx rgba(52, 143, 184, 0.18);
}
...
/* Detail readability polish: keep glass style but improve contrast. */
.chrome {
	background: linear-gradient(180deg, rgba(255, 255, 255, 0.72) 0%, rgba(255, 255, 255, 0.2) 72%, rgba(255, 255, 255, 0) 100%);
}
...
.hero-mask {
	background: linear-gradient(180deg, rgba(255, 255, 255, 0.04) 0%, rgba(255, 255, 255, 0.14) 48%, rgba(255, 255, 255, 0.24) 100%);
}
...
.cta-panel {
	...
	box-shadow: 0 18rpx 42rpx rgba(67, 112, 142, 0.15);
}

.action-pill {
	background: rgba(255, 255, 255, 0.82);
	border-color: rgba(190, 224, 240, 0.86);
}

.action-pill.on {
	background: rgba(220, 247, 251, 0.88);
	border-color: rgba(88, 189, 210, 0.36);
}
...
.tag-chip,
.mode-icon {
	background: rgba(219, 244, 251, 0.86);
	border-color: rgba(88, 189, 210, 0.34);
}
...
.hero-name {
	color: #ffffff;
	text-shadow: 0 6rpx 20rpx rgba(8, 18, 30, 0.3);
}
...
.lead-label {
	color: #1f6f87;
	background: rgba(219, 244, 251, 0.9);
	border-color: rgba(88, 189, 210, 0.34);
}

.lead-meta-item {
	background: rgba(236, 247, 252, 0.82);
	border-color: rgba(88, 189, 210, 0.18);
}

.opening-preview {
	background:
		linear-gradient(135deg, rgba(236, 247, 252, 0.96) 0%, rgba(255, 240, 247, 0.92) 100%),
...
.cta-shell {
	background: linear-gradient(180deg, rgba(255, 244, 248, 0) 0%, rgba(255, 244, 248, 0.16) 38%, rgba(255, 244, 248, 0.26) 100%);
}
