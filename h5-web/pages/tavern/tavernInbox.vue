<template>
	<view class="page" :class="localeFontClass">
		...
		<view class="chat-switch">
			<view class="chat-switch-btn active">
				<text class="cuIcon-messagefill chat-switch-ico"></text>
				<text>AI会话</text>
			</view>
			<view v-if="showHumanChatEntry" class="chat-switch-btn chat-switch-btn--human" @tap="openHumanChatList">
				<text class="cuIcon-peoplefill chat-switch-ico"></text>
				<text>真人聊天</text>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openHumanChatList() {
				const config = this.socialFeatureConfig || tavernApi.getSocialFeatureConfig();
...
<style scoped lang="scss">
	...
	.chat-switch {
		...
		box-shadow: 0 12rpx 26rpx rgba(38, 57, 77, 0.08);
	}
	...
	.chat-switch-btn.active {
		color: #fff;
		background: linear-gradient(135deg, #4f93a3 0%, #67b7d6 58%, #f5a7c3 100%);
		box-shadow: 0 12rpx 24rpx rgba(87, 169, 206, 0.18);
	}

	.chat-switch-btn--human {
		color: #2e6b4d;
	...
	/* Conversation list: remove the heavy card shell and keep the cover/title rhythm. */
	.sub-bar {
	...
	.card {
		...
		border-radius: 0;
		box-shadow: none;
		border-bottom: 1rpx solid rgba(36, 75, 102, 0.1);
		overflow: visible;
	}
