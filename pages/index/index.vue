		...
		>
			<view class="disc-header" :style="{ paddingTop: statusBarH + 'px' }">
				<view class="top-bar">
					...
					<text class="app-title">{{ discoverUi.title }}</text>
					<view class="top-bar-side top-bar-right">
						<text class="icon-more" @tap="onMoreMenu">...</text>
					</view>
				</view>
				...
				<view class="main-tabs">
					<view
						class="main-pill"
						:class="{ on: swiperCurrent === 0 }"
						@tap="setMainTab(0)"
					>{{ discoverUi.all }}</view>
					<view
						class="main-pill"
						:class="{ on: swiperCurrent === 1 }"
						@tap="setMainTab(1)"
					>{{ discoverUi.unlocked }}</view>
				</view>
</view>

			<view v-show="swiperCurrent === 0" class="page-box">
				<view v-if="isJgDiscover && discoverLoading" class="discover-skeleton">
					...
					<view class="grid2-item" v-for="c in displayGridList" :key="c.id" @tap="goDetail(c.id)">
						...
						>
							...
							<view class="card-meta">
								<view v-if="cardMetaBadges(c).length" class="meta-badges">
									<text
				...
				<view v-if="hasMoreDiscoverCards" class="list-more">
					<text class="list-more-text">{{ listProgressText(displayGridList.length, sortedDiscoverList.length) }}</text>
				</view>
		...
		methods: {
			...
			refreshNoticeUnread() {
				...
				return tavernNoticeState.fetchUnreadState(tavernApi, 30)
					...
					.catch(() => {
						const fallback = Math.max(0, Number(this.$store.state.unreadTotal) || 0);
			...
			tryLoadCharsFromBackend(options) {
				...
				const params = this.buildDiscoverParams();
				...
				tavernApi
					...
					.then((list) => {
						...
						this.allChars = Array.isArray(list) ? list : [];
			...
			setFeedTab(i) {
				...
				this.feedTab = i;
				if (!this.allChars.length && !this.discoverLoading && i !== 4) {
					this.tryLoadCharsFromBackend({ force: true });
					return;
				}
				this.syncDiscoverVisibleCounts();
...
<style lang="scss" scoped>
	...
	.disc-header {
		...
		background: linear-gradient(180deg, #1a1a28 0%, $tavern-surface-dark 100%);
	}
	...
	.mail-badge {
		...
		font-weight: 700;
		line-height: 1;
		box-shadow: 0 8rpx 18rpx rgba(236, 72, 153, 0.28);
		border: 2rpx solid rgba(18, 18, 27, 0.92);
	}
	...
	.search-clear {
		...
		border: 1rpx solid rgba(244, 114, 182, 0.2);
	}
	...
	.tag-chip.on {
		color: #fff;
		font-weight: 700;
		background: linear-gradient(135deg, rgba(126, 34, 206, 0.98) 0%, rgba(236, 72, 153, 0.94) 100%);
		border-color: rgba(244, 114, 182, 0.6);
		box-shadow: 0 10rpx 24rpx rgba(168, 85, 247, 0.24);
	}
	...
	.discover-menu-overlay {
		...
		z-index: 998;
		background:
			linear-gradient(180deg, rgba(6, 8, 16, 0.08) 0%, rgba(6, 8, 16, 0.22) 24%, rgba(6, 8, 16, 0.02) 58%);
	}
	...
	.discover-menu-card {
		...
		border-radius: 28rpx;
		background:
			linear-gradient(180deg, rgba(24, 27, 40, 0.98) 0%, rgba(15, 18, 30, 0.98) 100%);
		border: 1rpx solid rgba(255, 255, 255, 0.08);
		box-shadow:
			0 26rpx 52rpx rgba(4, 8, 18, 0.48),
	...
	.main-pill.on {
		color: #fff;
		font-weight: 700;
		background: linear-gradient(135deg, rgba(126, 34, 206, 0.82) 0%, rgba(168, 85, 247, 0.48) 100%);
		border: 1rpx solid rgba(192, 132, 252, 0.48);
		box-shadow: 0 8rpx 18rpx rgba(126, 34, 206, 0.2);
	}
	...
	.card-top {
		...
		box-shadow: 0 18rpx 48rpx rgba(5, 8, 20, 0.32);
	...
	.g3-inner {
		...
		background:
			...
			linear-gradient(180deg, rgba(29, 32, 47, 0.96) 0%, rgba(12, 14, 24, 0.98) 100%);
		box-shadow:
			0 18rpx 34rpx rgba(8, 11, 22, 0.26),
			0 6rpx 16rpx rgba(8, 11, 22, 0.18);
	...
	.g3-mask {
		...
		background: linear-gradient(0deg, rgba(11, 14, 24, 0.96) 0%, rgba(11, 14, 24, 0.12) 100%);
	}
	...
	.card-disc {
		...
		border: 1rpx solid rgba(255, 255, 255, 0.07);
		box-shadow:
			0 20rpx 42rpx rgba(6, 10, 20, 0.28),
			0 8rpx 18rpx rgba(6, 10, 20, 0.14);
	...
	.card-visual {
		...
		background:
			...
			linear-gradient(180deg, rgba(26, 29, 45, 0.78) 0%, rgba(13, 17, 30, 0.98) 100%);
	}
	...
	.card-visual::after {
		...
		z-index: 1;
		background:
			linear-gradient(180deg, rgba(10, 12, 22, 0.02) 0%, rgba(10, 12, 22, 0.03) 28%, rgba(10, 12, 22, 0.26) 72%, rgba(10, 12, 22, 0.76) 100%);
		pointer-events: none;
	}
	...
	.card-visual-copy {
		...
		background: linear-gradient(180deg, rgba(12, 14, 24, 0) 0%, rgba(8, 10, 18, 0.94) 100%);
		transition: transform 0.24s ease;
	}
	...
	.meta-foot-stat {
		...
		padding: 6rpx 14rpx;
		border-radius: 999rpx;
		background: linear-gradient(135deg, rgba(124, 58, 237, 0.2) 0%, rgba(219, 39, 119, 0.18) 100%);
		border: 1rpx solid rgba(192, 132, 252, 0.22);
	}
	...
	.meta-badge--vip {
		...
		color: #fde68a;
		border: 1rpx solid rgba(250, 204, 21, 0.22);
	}
	...
	.meta-badge--mode {
		...
		color: #c7d2fe;
		border: 1rpx solid rgba(129, 140, 248, 0.22);
	}
...
.group-preview-card {
	...
	border-radius: 30rpx;
	background:
		linear-gradient(145deg, rgba(29, 33, 49, 0.96) 0%, rgba(20, 22, 36, 0.96) 42%, rgba(41, 20, 52, 0.92) 100%),
		radial-gradient(circle at top right, rgba(236, 72, 153, 0.18), transparent 34%);
	border: 1rpx solid rgba(255, 255, 255, 0.08);
	box-shadow:
		0 24rpx 58rpx rgba(5, 8, 20, 0.34),
...
.group-preview-action {
	...
	box-shadow: 0 18rpx 34rpx rgba(147, 51, 234, 0.26);
}
