...
// ===== dev tooling: capture /generate request body for Golden Diff =====
const ST_DEBUG_GENERATE_BODY = String(process.env.ST_DEBUG_GENERATE_BODY || '').toLowerCase() === 'true';
const ST_DEBUG_GENERATE_BODY_MAX_CHARS = (() => {
    const raw = Number.parseInt(String(process.env.ST_DEBUG_GENERATE_BODY_MAX_CHARS || ''), 10);
    return Number.isFinite(raw) && raw > 0 ? raw : 20000;
})();
const ST_DEBUG_GENERATE_BODY_MAX_MESSAGE_CHARS = (() => {
    const raw = Number.parseInt(String(process.env.ST_DEBUG_GENERATE_BODY_MAX_MESSAGE_CHARS || ''), 10);
    return Number.isFinite(raw) && raw > 0 ? raw : 400;
})();

// Persist the latest /generate body per-user so runtime-chat can do golden diffs without manual copying.
const ST_DEBUG_GOLDEN_DIFF = String(process.env.ST_DEBUG_GOLDEN_DIFF || '').toLowerCase() === 'true';
const ST_DEBUG_GOLDEN_DIFF_MAX_CHARS = (() => {
    const raw = Number.parseInt(String(process.env.ST_DEBUG_GOLDEN_DIFF_MAX_CHARS || ''), 10);
    return Number.isFinite(raw) && raw > 0 ? raw : 2_000_000;
})();
const ST_DEBUG_GOLDEN_DIFF_MAX_MESSAGE_CHARS = (() => {
    const raw = Number.parseInt(String(process.env.ST_DEBUG_GOLDEN_DIFF_MAX_MESSAGE_CHARS || ''), 10);
    return Number.isFinite(raw) && raw > 0 ? raw : 10_000;
})();

/**
 * Sanitizes a /generate request body for logging (redacts secrets and truncates large fields).
 * @param {any} body
 * @returns {any}
 */
function sanitizeGenerateBodyForLog(body) {
    try {
        const clone = body == null ? {} : JSON.parse(JSON.stringify(body));
        if (clone && typeof clone === 'object') {
            // Redact common secret fields
            if (typeof clone.proxy_password === 'string' && clone.proxy_password.length) clone.proxy_password = '[REDACTED]';
            if (typeof clone.api_key === 'string' && clone.api_key.length) clone.api_key = '[REDACTED]';
            if (typeof clone.authorization === 'string' && clone.authorization.length) clone.authorization = '[REDACTED]';

            // Truncate messages to avoid dumping entire prompts
            if (Array.isArray(clone.messages)) {
                clone.messages = clone.messages.map(m => {
                    if (!m || typeof m !== 'object') return m;
                    const out = { ...m };
                    if (typeof out.content === 'string' && out.content.length > ST_DEBUG_GENERATE_BODY_MAX_MESSAGE_CHARS) {
                        out.content = out.content.slice(0, ST_DEBUG_GENERATE_BODY_MAX_MESSAGE_CHARS) + '...(truncated)';
                    }
                    // Some providers use other fields (tool calls / reasoning etc.)
                    if (typeof out.reasoning === 'string' && out.reasoning.length > ST_DEBUG_GENERATE_BODY_MAX_MESSAGE_CHARS) {
                        out.reasoning = out.reasoning.slice(0, ST_DEBUG_GENERATE_BODY_MAX_MESSAGE_CHARS) + '...(truncated)';
                    }
                    return out;
                });
            }
        ...
        return { error: true, note: 'sanitize failed', type: typeof body };
    }
}

/**
 * Sanitizes a /generate request body for saving to disk (redacts secrets; truncates only if huge).
 * @param {any} body
 * @returns {any}
 */
function sanitizeGenerateBodyForSave(body) {
    try {
        const clone = body == null ? {} : JSON.parse(JSON.stringify(body));
        if (clone && typeof clone === 'object') {
            if (typeof clone.proxy_password === 'string' && clone.proxy_password.length) clone.proxy_password = '[REDACTED]';
            ...
            if (Array.isArray(clone.messages)) {
                clone.messages = clone.messages.map(m => {
                    if (!m || typeof m !== 'object') return m;
                    const out = { ...m };
                    if (typeof out.content === 'string' && out.content.length > ST_DEBUG_GOLDEN_DIFF_MAX_MESSAGE_CHARS) {
                        out.content = out.content.slice(0, ST_DEBUG_GOLDEN_DIFF_MAX_MESSAGE_CHARS) + '...(truncated)';
                    }
                    if (typeof out.reasoning === 'string' && out.reasoning.length > ST_DEBUG_GOLDEN_DIFF_MAX_MESSAGE_CHARS) {
                        out.reasoning = out.reasoning.slice(0, ST_DEBUG_GOLDEN_DIFF_MAX_MESSAGE_CHARS) + '...(truncated)';
                    }
                    return out;
                });
            }
...
function tryPersistGoldenGenerateBody(request) {
    try {
        const directories = request?.user?.directories;
        if (!directories?.user) return;
        const debugDir = path.join(directories.user, 'st-debug');
        const shouldPersist = ST_DEBUG_GOLDEN_DIFF || fs.existsSync(debugDir);
        if (!shouldPersist) return;
        if (!fs.existsSync(debugDir)) {
            fs.mkdirSync(debugDir, { recursive: true });
        }
        const source = String(
            request?._stDebugSource ||
            request?.originalUrl ||
            request?.url ||
            '/api/backends/chat-completions/generate',
        ).trim() || '/api/backends/chat-completions/generate';
        const rawTag = String(
            request?._stDebugCaptureTag ||
            (source === '/api/runtime/chat/generate' ? 'runtime' : 'browser'),
        ).trim().toLowerCase();
        const tag = rawTag.replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '') || 'generic';
        const outPaths = [
            path.join(debugDir, 'last-generate-body.json'),
            path.join(debugDir, `last-generate-body-${tag}.json`),
        ];
        const payload = {
            captured_at: new Date().toISOString(),
            source,
            tag,
            body: sanitizeGenerateBodyForSave(request.body),
        };
        let dumped = JSON.stringify(payload);
        if (dumped.length > ST_DEBUG_GOLDEN_DIFF_MAX_CHARS) {
            payload.body = sanitizeGenerateBodyForLog(request.body);
            dumped = JSON.stringify(payload);
            if (dumped.length > ST_DEBUG_GOLDEN_DIFF_MAX_CHARS) {
                dumped = dumped.slice(0, ST_DEBUG_GOLDEN_DIFF_MAX_CHARS) + '...(truncated)';
            }
        }
        for (const outPath of [...new Set(outPaths)]) {
            fs.writeFileSync(outPath, dumped, 'utf8');
        }
    } catch (err) {
        console.warn('[st-debug] Failed to persist /generate body:', err?.message ?? err);
    }
}
...
router.post('/status', async function (request, statusResponse) {
    try {
        ...
        if (request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.OPENAI) {
            ...
            apiUrl = new URL(request.body.reverse_proxy || API_OPENROUTER).toString();
            apiKey = request.body.reverse_proxy ? request.body.proxy_password : readSecret(request.user.directories, SECRET_KEYS.OPENROUTER);
            ...
            apiKey = request.body.proxy_password || readSecret(request.user.directories, SECRET_KEYS.CUSTOM);
            headers = {};
            ...
            apiUrl = new URL(request.body.reverse_proxy || API_GROQ).toString();
            apiKey = request.body.reverse_proxy ? request.body.proxy_password : readSecret(request.user.directories, SECRET_KEYS.GROQ);
            headers = {};
            ...
            apiUrl = new URL(request.body.reverse_proxy || API_FIREWORKS).toString();
            apiKey = request.body.reverse_proxy ? request.body.proxy_password : readSecret(request.user.directories, SECRET_KEYS.FIREWORKS);
            headers = {};
...
export async function handleGenerateRequest(request, response) {
    try {
        if (!request.body) return response.status(400).send({ error: true });

        tryPersistGoldenGenerateBody(request);

        if (ST_DEBUG_GENERATE_BODY) {
            const sanitized = sanitizeGenerateBodyForLog(request.body);
            let dumped = '';
            try {
                dumped = JSON.stringify(sanitized);
            } catch {
                dumped = String(sanitized);
            }
            if (dumped.length > ST_DEBUG_GENERATE_BODY_MAX_CHARS) {
                dumped = dumped.slice(0, ST_DEBUG_GENERATE_BODY_MAX_CHARS) + '...(truncated)';
            }
            console.log('[st-debug] /api/backends/chat-completions/generate body:', dumped);
        }
